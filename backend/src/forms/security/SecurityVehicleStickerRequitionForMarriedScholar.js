...
const renderSecurityVehicleStickerRequitionForMarriedScholarPdf = (doc, submission) => {
  ...
  const supervisorRecommendation = String(getResponseValue(responses, "supervisorRecommendation") || "").trim();
  const hodRemarks = String(getResponseValue(responses, "hodRemarks") || "").trim();
  ...
  doc
    ...
    .text("REQUISITION FOR VEHICLE STICKER (Resident of Married Accommodation Only)", {
      align: "center",
    });
  ...
  const mainRowHeights = [84, 60, 88, 102, 88];
  ...
  drawCellText(vehicleOwnership, mainX + labelColW, row1Y + 36, mainW - labelColW, {
    fontSize: bodySize,
    lineGap: 1,
  });

  const row2Y = row1Y + mainRowHeights[0];
  drawCellText("Roll No.: -\n\nDepartment: -", mainX, row2Y, labelColW, {
    fontSize: bodySize,
    lineGap: 2,
  });
  drawCellText(rollNo, mainX + labelColW, row2Y + 4, mainW - labelColW, { fontSize: bodySize });
  drawCellText(department, mainX + labelColW, row2Y + 36, mainW - labelColW, { fontSize: bodySize });
  ...
  const recY = mainY + mainH + 30;
  const recX = mainX;
  const recW = mainW;
  const recRowHeight = 47;
  const recH = recRowHeight * 2;
  const recLabelW = Math.round(recW * 0.51);

  doc.rect(recX, recY, recW, recH).lineWidth(0.8).stroke();
  drawLine(recX, recY + recRowHeight, recX + recW, recY + recRowHeight);
  drawLine(recX + recLabelW, recY, recX + recLabelW, recY + recH);

  drawCellText("Recommendation by Supervisor", recX, recY, recLabelW, {
    fontSize: pdfStyles.getFontSize("SECTION_HEADER"),
  });
  drawCellText("Remarks by HoD", recX, recY + recRowHeight, recLabelW, {
    fontSize: pdfStyles.getFontSize("SECTION_HEADER"),
  });

  drawCellText(supervisorRecommendation, recX + recLabelW, recY, recW - recLabelW, {
    fontSize: bodySize,
  });
  drawCellText(hodRemarks, recX + recLabelW, recY + recRowHeight, recW - recLabelW, {
    fontSize: bodySize,
  });
...
module.exports = { renderSecurityVehicleStickerRequitionForMarriedScholarPdf };